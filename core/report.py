...
def write_pdf(data: ReportData, path: str | Path) -> Path:
    ...
    flow.append(Paragraph(
        f"Sequential-optimization diagnostic report · generated "
        f"{datetime.now().astimezone().strftime('%Y-%m-%d %H:%M')}", SMALL))
    flow.append(Spacer(1, 6 * mm))

    if data.gate_bypassed:
        flow.append(Table(
            [[Paragraph("CAUTION — generated with requirements unmet. "
    ...
    # ── gate verdict ───────────────────────────────────────────────
    flow.append(Paragraph("Gate verdict", H2))
    verdict_rows = [["", "requirement", "verdict", "evidence"]]
    r2_txt = f"R² = {data.loocv.r2:+.3f}" if data.loocv else "not computed"
    ...
    verdict_rows += [
        ...
        ["④", "replicates", MARK.get(g.replicates, "—"),
         f"conditions with replicates {n_rep}/{ds.n_conditions}"],
    ]
    ...
    flow.append(Paragraph(
        ("<b>Recommendation LOCKED</b> — " + " · ".join(g.reasons)) if g.locked
        else "<b>Requirements met</b> — next-candidate recommendation is available.",
        style("V", 9.5, 14, "#b71c1c" if g.locked else "#2e7d32")))
    flow.append(Spacer(1, 6 * mm))

    # ── raw-data summary ───────────────────────────────────────────
    flow.append(Paragraph("Raw data", H2))
    summary = [
        ["conditions (usable / total)", f"{ds.n_conditions} / {ds.n_conditions_all}"],
        ["measurements", f"{ds.n_measurements}"],
        ["replicate distribution", str(ds.rep_distribution)],
    ...
    flow.append(PageBreak())
    flow.append(Paragraph("Calculation log", H2))
    flow.append(Paragraph(
    ...
    for line in calculation_log(data).splitlines():
        flow.append(Paragraph(line.replace(" ", "&nbsp;").replace("<", "&lt;") or "&nbsp;", MONO))
    ...
    def _stamp(canvas, doc):
        ...
        if data.gate_bypassed:
            ...
            canvas.rotate(28)
            canvas.drawCentredString(0, 0, "REQUIREMENTS UNMET")
        canvas.restoreState()

    SimpleDocTemplate(str(path), pagesize=A4,
                      leftMargin=22 * mm, rightMargin=22 * mm,
                      topMargin=18 * mm, bottomMargin=18 * mm,
                      title=f"{p.name} — diagnostic report").build(
